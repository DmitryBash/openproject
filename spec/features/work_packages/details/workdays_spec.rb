...
describe 'Work packages datepicker workdays',
  ...
  context 'with default work days' do
    ...
    it 'shows them as disabled' do
      ...
      weekend_days.each do |weekend_day|
        expect(page).to have_selector('.dayContainer:first-of-type .flatpickr-day.flatpickr-disabled',
                                      text: weekend_day,
                                      exact_text: true)
      end

      ((1..31).to_a - weekend_days).each do |workday|
        expect(page).to have_selector('.dayContainer:first-of-type .flatpickr-day:not(.flatpickr-disabled)',
                                      text: workday,
                                      exact_text: true)
      end
    end
  ...
  context 'with all days marked as weekend' do
    ...
    it 'shows them as disabled' do
      expect(page).to have_selector('.dayContainer', count: 2)
      expect(page).to have_selector('.dayContainer:first-of-type .flatpickr-day.flatpickr-disabled', count: 31)
    end
  end
end
